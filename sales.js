export const productSalesOrgMapping = (payload = {}) => {
  const records = payload.value || [];

  return {
    productSalesOrgs: records
      // equivalent to: payload filter($.to_SalesDelivery != null)
      .filter(
        po =>
          Array.isArray(po._ProductSalesDelivery) &&
          po._ProductSalesDelivery.length > 0
      )
      // flatten(...)
      .flatMap(po =>
        po._ProductSalesDelivery
          // SalesMeasureUnit & BaseUnit filtering
          .filter(
            pso =>
              (["EA", "BX", ""].includes(pso.SalesMeasureUnit)) &&
              (["EA", "BX"].includes(pso.BaseUnit))
          )
          .map(pso => ({
            materialNum: pso.Product,

            distributionChannel: pso.ProductDistributionChnl,

            salesStatus: pso.ProductSalesStatus,

            modStatus:
              pso.ProductHasAttributeID01 === true &&
              pso.ProductSalesStatus === "D2"
                ? "ACTIVE"
                : pso.ProductHasAttributeID01 === true &&
                  pso.ProductSalesStatus === "D3"
                ? "DISCONTINUED"
                : "NOTAPPLICABLE",

            salesOrgCode: pso.ProductSalesOrg,

            itemCategoryGroup: pso.ItemCategoryGroup,

            baseUnit: pso.BaseUnit,

            minimumOrderQuantity: pso.MinimumOrderQuantity,

            salesUnitOfMeasure: pso.SalesMeasureUnit,

            productCatalogCode: po.MsBookPartNumber || "",

            materialBaseNum: po.BasicProduct || ""
          }))
      )
  };
};
...
const salesOrgResult = productSalesOrgMapping(payload);
console.log(JSON.stringify(salesOrgResult, null, 2));
